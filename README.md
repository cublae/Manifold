...
Ничего включать не нужно: если Stylix есть в конфигурации и включён, модуль сам
это увидит.

Большая часть приходит и без интеграции. Manifold рисует именованными цветами
libadwaita, а GTK-цель Stylix записывает их в `~/.config/gtk-4.0/gtk.css` —
всю палитру вместе с акцентом; шрифт приезжает там же через `gtk-font-name`.
GTK отдаёт и то и другое шеллу, как любому другому приложению.

Единственное, что для этого нужно — **не задавать `theme.accent`**. Заданный
акцент закрашивает собой то, что выбрал Stylix; по умолчанию он не задан.

Интеграция добавляет две вещи, которых в GTK нет: полярность (иначе шелл
спросил бы портал рабочего стола и мог получить ответ, расходящийся с палитрой,
которой он рисует) и путь к обоям (иначе его пришлось бы искать через waypaper
или swww).

Всё, что задано в `programs.manifold.theme`, перебивает Stylix — разойтись с
одним углом схемы стоит одной строки:

```nix
programs.manifold.theme.mode = "light";   # вопреки stylix.polarity
programs.manifold.stylix.enable = false;  # или совсем не следовать
```

### Горячие клавиши

Команда не поднимает второй шелл, а уходит работающему по шине — около 10 мс,
так что бинд отрабатывает без заметной задержки и биндить его можно напрямую,
без обёрток:
...
Если шелл не запущен, команда так и скажет и завершится с ненулевым кодом, а не
поднимет его: запуск — это `manifold` без аргументов, и только он.
...
| `theme.accent` | `theme.accent` | `null` | Акцентный цвет, hex, только для поверхностей Manifold. `null` не перекрывает ничего — тогда берётся акцент рабочего стола: настройка GNOME или то, что генератор темы вроде Stylix записал в `~/.config/gtk-4.0/gtk.css`. Без такой темы применится собственный акцент libadwaita, так что на голой системе вид не меняется |
...
| `programs.manifold.stylix.enable` | включён, если включён Stylix | Следовать Stylix: полярность и обои. Цвета и шрифт приходят сами, через его GTK-цель — при условии, что `theme.accent` не задан. Опции `theme.*` перебивают |
